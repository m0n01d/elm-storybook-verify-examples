let fileRegex = /src\/(Stories|VerifyExamples)\/(.*)\.elm$/;
...
module.exports = () => ({
  ...
  transform: async (rawSource, id) => {
    if (fileRegex.test(id)) {
      let root = fileRegex.exec(id);
      console.log(fileRegex.exec(id));
      let modulePath = root[2];
      ...
      try {
        console.log = () => {};
        compiledJs = await compiler.compileToString(id, {
          optimize: false,
          debug: false,
        });
        console.log = log;
        ...
        throw (
          "Elm found an error!\n\n" +
          elmCompilerError.message.slice(
            "Compilation failed\nCompiling ...".length
          )
        );
      }

      let transformedCode = compiledJs
...
export const ${finalModuleName} = (controls) => {
  ...
  window.requestAnimationFrame(() => {
    let app = globalThis.Elm.${root[1]}.${fullModuleName}.init({ node })
